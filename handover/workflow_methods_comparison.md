...
| タスク例 | 理由 |
|---|---|
| det_max(n) の上界・下界の理論的考察 | 数学的推論が必要 |
| Hadamard 予想との関係の議論 | 深い文脈理解が必要 |
| 新しい極値行列構成法の設計 | 探索的な発見プロセス |

### Method B を使うべきタスク

| タスク例 | 理由 |
|---|---|
| 行列式計算ライブラリの実装 | コード生成＋テスト実行 |
| n=3,4 の全数探索スクリプト | 実行結果を確認しながら開発 |
| JSON スキーマの定義と I/O | 既存コードとの整合性を確認 |
| 分布の可視化スクリプト | イテレーティブな開発 |

### Method C を使うべきタスク（導入後）

| タスク例 | 理由 |
|---|---|
| 型アノテーションの追加 | 機械的、テストで検証可能 |
| docstring の統一 | テンプレート的な作業 |
| lint 警告の修正 | 機械的な修正 |
...
本プロジェクトでは、行列式計算の正しさが結論の基礎であり、コードの正しさ＝数学的正しさである。
